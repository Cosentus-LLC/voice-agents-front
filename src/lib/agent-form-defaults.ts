...
export function agentDefaultsFromSchema(schema: AgentSchema): Omit<Agent, "id" | "name" | "created_at" | "updated_at"> {
  ...
  return {
    ...
    ivr_goal: str(d.ivr_goal, ""),
    identity_verification_keys: strArr(d.identity_verification_keys),
    call_kind: d.call_kind === "payer" || d.call_kind === "patient" ? d.call_kind : null,
    required_fields: strArr(d.required_fields),
    success_criteria: strArr(d.success_criteria),
    speak_first: bool(d.speak_first, true),
